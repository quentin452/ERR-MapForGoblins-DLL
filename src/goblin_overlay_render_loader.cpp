...
namespace goblin::overlay_render_loader
{
    ...
    namespace
    {
        ...
        using DrawRunHudFn = void (*)(const goblin::overlay::OverlayFrameCtx *);
        ...
        struct RenderExports
        {
            ...
            DrawRunHudFn draw_run_hud = nullptr;
            ...
            RebuildMarkersFn rebuild_markers = nullptr;
        ...
        bool copy_and_load(unsigned gen, RenderExports &out)
        {
            ...
            e.draw_run_hud = reinterpret_cast<DrawRunHudFn>(GetProcAddress(m, "MFG_DrawRunHud"));
            ...
            e.rebuild_markers = reinterpret_cast<RebuildMarkersFn>(GetProcAddress(m, "MFG_RebuildMarkers"));
            e.service_pending_warp =
    ...
    void call_draw_run_hud(const goblin::overlay::OverlayFrameCtx &ctx)
    {
        SharedGuard g;
        if (g_cur.draw_run_hud) g_cur.draw_run_hud(&ctx);  // null-guarded: absent in an old module
    }
    ...
    {
        SharedGuard g;
        if (g_cur.rebuild_markers) g_cur.rebuild_markers();
    }
    void call_service_pending_warp()
    {
        SharedGuard g;
        if (g_cur.service_pending_warp) g_cur.service_pending_warp();
    }
    std::string call_build_far_relief(int group, int cellSize)
    {
        SharedGuard g;
        char b[192] = {};
        if (g_cur.build_far_relief) g_cur.build_far_relief(group, cellSize, b, (int)sizeof(b));
        return b;
    }
    ...
    void call_draw_run_hud(const goblin::overlay::OverlayFrameCtx &ctx) { goblin::overlay::draw_run_hud(ctx); }
    ...
    void call_rebuild_markers() { goblin::worldmap::rebuild_markers(); }
