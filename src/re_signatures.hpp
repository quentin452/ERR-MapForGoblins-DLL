...
namespace goblin::sig
{
    ...
    // +0x78(Z). Distinct from CSWorldGeomMan (WORLD_GEOM_MAN_SLOT), which reads origin
    // underground. The fn prologue ends with `mov rax,[rip+DAT_143d69ba8]` (48 8B 05);
    // relative_offsets {{24,28}} → slot from that rip-disp. (RE: underground_player_pos.)
