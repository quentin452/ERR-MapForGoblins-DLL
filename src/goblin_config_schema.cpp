...
namespace goblin::config
{
    ...
    bool    spiderfyHoldCtrl = true; // require Ctrl held to OPEN the fan (so panning doesn't pop fans)
    ...
    std::vector<IniSection> build_schema()
    {
        return {
            {"Goblin", nullptr, false, {
                ...
                IniEntry{"overlay_toggle_gamepad", IniType::GamepadMask, &cfg::overlayToggleGamepad, "Y+R3",
                         "Gamepad combo that opens/closes the in-game overlay menu. Default: Y+R3.\n"
            ...
            {"Clustering",
             ...
             false, {
                ...
                B("spiderfy_hold_ctrl", spiderfyHoldCtrl, "true",
                  "Require Ctrl held to OPEN the spiderfy fan, so dragging/panning the map\n"
                  "doesn't pop fans as the cursor sweeps clusters. Once open it stays until\n"
                  "the cursor leaves (Ctrl not needed to keep it). Off = open on plain hover."),
                ...
                B("cluster_debug_markers", clusterDebugMarkers, "false",
                  "Overlay DEBUG: draw a small dot on every marker coloured by its projection state\n"
                  "(green = live-projected, red = baked-fallback, grey = untried) plus its map-space\n"
                  "tile, so you can see why some items don't cluster. Off by default."),
            ...
            {"Markers",
             "Overlay marker rendering (F1 > Marker scale). Real user preferences: overall\n"
             "sizing + the legibility/altitude/motion-sync behaviour. The former dev-era\n"
             "CALIBRATION sliders (grace/symbol/cluster scale, grace offset, altitude\n"
             "deadzone) were hardcoded final in the settings sweep and are no longer keys.",
             false, {
            ...
            {"Minimap",
             "In-game minimap HUD (F1 > Minimap). A small north-up minimap in a screen\n"
             "corner showing nearby markers during gameplay (overworld only for now).",
             false, {
                ...
                IniEntry{"minimap_size", IniType::F32, &cfg::minimapSize, "100",
                  "Minimap radius in pixels. 100 = default (down from 130, user-tuned 2026-07-01).\nBUGFIX 2026-07-01: this and several other F32 settings used to silently get\nclamped to a blanket [0.1, 5.0] range on load meant for the overlay SCALE\nmultipliers -- 100 would become 5 on the very next load after any save. Fixed\nwith a real per-field range.",
                  false, nullptr, 24.0f, 500.0f},
                ...
                IniEntry{"minimap_anchor_right", IniType::Bool, &cfg::minimapAnchorRight, "true",
                  "Minimap corner: anchor to the RIGHT edge (false = left). Default true.",
                  false, nullptr},
                IniEntry{"minimap_anchor_bottom", IniType::Bool, &cfg::minimapAnchorBottom, "false",
            ...
            {"Debug",
             ...
             false, {
                ...
                IniEntry{"probe_field_spec", IniType::String, &cfg::probeFieldSpec, "",
                         "Target for probe_field_access: ParamName:rowId:offset[:len[:rw]] (len\n1|2|4|8 default 4; rw r|w default r). The DLL resolves the LIVE row via the\nparam chain, so the spec is session-independent. Examples:\nEquipParamGoods:1000:0x72:1:r (sortGroupId on the Crimson flask -- open the\ninventory to trigger) or AssetEnvironmentGeometryParam:99821:0xb8:4:r\n(pickUpItemLotParamId -- loot the matching collectible).\nRAW geom mode: @geom[:offset[:len[:rw]]] (default +0x263:1:r) watches a LIVE\nCSWorldGeomIns alive byte instead of a param row -- for finding the native\ncollected/'is-geom-open' getter. The collected WGM walk arms DR0 on the first\ntracked ALIVE gather/piece instance ([GEOMPROBE] logs which one); go collect/\nreload that asset and the [FWA] hit RIP is the getter. Only used when\nprobe_field_access is on.", false, nullptr},
