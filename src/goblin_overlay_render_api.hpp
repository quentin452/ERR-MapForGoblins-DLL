...
#define GOBLIN_CFG_BOOL_LIST(X) \
    ...
    X(diagLootPos) X(debugLogging) X(anonymousLoot) X(anonymousLootAggressive) X(anonymousLootAltitude) X(debugMarkerTooltip) X(dropMerchantPhantoms) X(redifyBossIcons) \
...
#undef GOBLIN_CFG_DECL_U16
    ...
    // Force a fresh marker/bucket build so a LIVE param edit (pickUpItemLotParamId repoint, a lot's
    ...
    GOBLIN_RENDER_API std::string virtual_map_find(const std::string &query);
