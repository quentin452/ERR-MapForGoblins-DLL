...
void draw_clustering(Filter &f)
{
    ...
    ImGui::SeparatorText(tr("Clustering"));
    ...
    if (en)
    {
        ImGui::TextDisabled("%s", tr("One mixed pile per location. Per-category opt-in above."));
        ...
        if (ImGui::Checkbox(tr("Distance-adaptive (cluster by distance from player)"), &da))
        {
        ...
        if (da)
        {
            ...
            if (ImGui::SliderInt(tr("Near radius (tiles)"), &nr, 1, 40))
            ...
            ImGui::SetNextItemWidth(160.0f);
            if (ImGui::SliderInt(tr("Far radius (tiles)"), &fr, 2, 80))
        ...
        ImGui::TextDisabled("%s", tr("Preset:"));
        ImGui::SameLine();
        if (ImGui::SmallButton(tr("Completionist")))
        {
