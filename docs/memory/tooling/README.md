...
## Dev / test infra
- **`toml::parse_file` empty under Proton (exceptions-ON only)** [active, gotcha] — returns an EMPTY
  table (silent) for a valid file under Wine ONLY in the default exceptions-ON config. `TOML_EXCEPTIONS=0`
  works (custom_items.cpp does this, verified fine). Fix = `ifstream` + `toml::parse(string)`. →
