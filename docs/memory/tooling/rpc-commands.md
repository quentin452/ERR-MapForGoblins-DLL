...
| `warp_local` | `warp_local <x> <y> <z>` | Teleport to a tile-local Havok pos (the `coords` `local=` frame). WORKS (SOLVED 2026-07-07): writes the player's HAVOK BODY Vec3 directly (`*(*(LocalPlayer+0x190)+0x68)+0x70/74/78`) via `goblin::warp::teleport_coords` — the move HOLDS (unlike a raw `+0x6C0` store, an output mirror physics reclaims → the old snap-back bug). Intra-region (a far target may hit unstreamed void — use `warp <graceId>` cross-map). |
| `warp_xyz` | `warp_xyz <worldX> <worldZ> [worldY]` | Teleport to a unified WORLD/marker XZ (converts world→tile-local then drives `teleport_coords`, same working body-write path as `warp_local`). Intra-region. Live-verified (+20 m held). |
