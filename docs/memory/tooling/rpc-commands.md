...
| `warp_local` | `warp_local <x> <y> <z>` | ⚠ **DOES NOT teleport in gameplay** (RE'd 2026-07-07). Writes `LocalPlayer+0x6C0` — but that field is an OUTPUT MIRROR the physics thread overwrites every frame (it's the engine's warp-STAGING buffer: the SetPos path `er+0xdc6380` writes +0x6C0 **and** sets the `+0x160 \| 0x80` pending flag **and** calls the propagate fn `er+0xdc6e90`; a raw write does only the first → snaps back next frame). Readback right after = target (write lands), but a later `coords` shows the old pos. **Proper teleport needs the engine SetPos call** (`er+0xdc6380`, extra r8/r9 args to RE) — follow-up. Use `warp <graceId>` for a working teleport meanwhile. |
| `warp_xyz` | `warp_xyz <worldX> <worldZ> [worldY]` | ⚠ Same as `warp_local` — **no teleport in gameplay** (writes the +0x6C0 mirror, physics reverts it). See `warp_local`. |
