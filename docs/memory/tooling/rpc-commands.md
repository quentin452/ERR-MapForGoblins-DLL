...
| `entrance_anchor` | `entrance_anchor set <srcArea> <dstArea> <dstGx> <dstGz> <dstPx> <dstPz>` \| `reset` \| `list` | **Slice-1** fallback dungeon-entrance anchor ([dungeon_entrance_fallback_anchor_plan](../../plans/dungeon_entrance_fallback_anchor_plan.md)). Registers an overworld ENTRANCE for a dungeon area that `legacy_fold` DECLINES (mod-added areas with no `WorldMapLegacyConvParam` row, e.g. ERR area 45), so its markers collapse onto the entrance instead of the block-local origin pile. `set` triggers a marker rebuild. Empty table = no-op (today's behaviour). Anchor is a `ConvRow` dst (entrance world = `dstGx*256+dstPx`, `dstGz*256+dstPz`). Seed source (real entrance pos) = Slice 2/3; this verb also serves as the manual (Slice-2b) seed. Verified 2026-07-23: `set 45 60 40 50 100 100` moved all area-45 markers to `w(10340,12900)`; `reset` restored the origin pile. |
| `vmap emevd` | `vmap emevd <mapName> [needle]` | **RE probe** ([dungeon_entrance_fallback_anchor_plan](../../plans/dungeon_entrance_fallback_anchor_plan.md) Slice 3): dump an EMEVD's bank-2000 `InitializeEvent` inits (via `read_game_file_decompressed("event/<mapName>.emevd.dcx")` + `msbe::emevd_inits`) to `[EMEVDUMP]`. No needle → a template-id histogram (what event templates the map uses); with a needle (entityId / template / any arg word, decimal or `0x`) → the full arg words of every init referencing it. Hunts the overworld→dungeon warp by its trigger entity / destination-map arg. NB only bank-2000 inits — a direct `WarpPlayer` (bank 2003) isn't captured yet (see the findings doc). |
...
| `vmap prov` | `vmap prov <lot\|item-name>` | **Item provenance.** For a lot id (numeric) or item-name substring, dump EVERY disk placement carrying that lot — `src=Treasure\|LOD-Treasure\|Collectible lot=<L> '<item>' \| RAW area<A> grid(gx,gz) pos(x,z) \| PROJ area<pa> world(wx,wz) <onmap\|OOB\|DECLINED-oob>` → `[PROV]` log. Reads cached `g_parsed` (no re-scan). The tool for "why is this item DUPLICATED / mis-projected": shows each physical source tile + where it lands. E.g. `vmap prov 'Codex of the All-Knowing'` = 3 placements (real m11_10 Roundtable + copies m31_90 declined-OOB, m33 onmap-dup) — ERR stores the Roundtable in several map slots; NPCs dedup by entity-id, loot doesn't. |
...
| `param_rows` | `param_rows <ParamName> [max]` | The param's REAL row ids + its true row count, enumerated from the live table. Use it before concluding anything about a param's contents: probing ids with `param_get` samples, it does not enumerate — a param numbered by map id looks empty to a 0..N sweep. `err ... (0 rows)` = not resident / absent. |
| `flag` | `flag <id>` / `flag range <lo> <hi>` / `flag set <id> <0\|1>` | Read, scan or write EVENT FLAGS live. `range` is the useful one: it prints every flag ON in a span (capped at 20000 ids, 200 reported) — scanning a boss's 1000-id tile band shows which flag the engine ACTUALLY set on its death, which the EMEVD cannot say when the flag is assigned from a parameter (see `docs/memory/features/run-tracker.md`). Every id reads 0 until the flag API is warm; probe `flag 6001` (AlwaysOn) first. Driver: `tools/rpc_tests/probe_boss_defeat_flags.py`. |
...
| `fold_probe` | `fold_probe <area> <gx> <gz> [px] [pz]` | Run the LIVE `legacy_fold` (`WorldMapLegacyConvParam`) and report `matched` + folded dst block + **`ent=(ent_x,ent_z)`** = the overworld ENTRANCE base point (`ConvRow.dst` world). Ground truth for a mapped dungeon's entrance; `matched=0 ent=(0,0)` = the area is DECLINED (no conv row, e.g. ERR area 45). Used to validate the Slice-2/3 [entrance-anchor](../../plans/dungeon_entrance_fallback_anchor_plan.md) source against known dungeons (a30→`(10913,8522)`, a32→`(11044,8380)`). |
