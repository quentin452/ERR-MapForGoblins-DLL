...
**BOUND — the flat array is DENSE-LOW-ID ONLY (2026-07-01).** `VirtualQueryEx` on the base gives the region size (~26 MB in the test save) → covers groups up to `regionsize/125` (~219152, i.e. flag ids up to ~219M). LOW flags (quest registers 3xxx/6xxx/9xxx, tunnel 32009xxx) are in-bounds and read correctly. **The 10-digit area/entity flags (`10AABBCDDD`, e.g. `1043379356`) are OUT of the flat array** — `base + group*125` for group ~1.04M points 130 MB past a 26 MB region = garbage that can read as a spurious `1`. They live in a separate sparse store (the `mgr+0x38` map has 11920 nodes incl. high groups, but its node value `{0x1cb,1,0x1cb,group}` is NOT a bit-block pointer — resolution NOT yet cracked). `read_event_flags.py` bounds-checks against the region and returns `None` for OOB. CONSEQUENCE: RPM can validate the low-flag quest REGISTERS live, but NOT the 10-digit gate/area flags — for those, advance the quest in-game and watch the register move, or read via the game's own `IsEventFlag`.
