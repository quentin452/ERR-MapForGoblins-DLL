...
**FULL LOOKUP CRACKED — reads ANY flag incl. 10-digit (2026-07-01, decompiled `FUN_1405F9400` @ RVA 0x5F9400, the fn `IsEventFlag`@0x5D1330 calls).** `base + group*125` is a SHORTCUT that only works when a group's block sits in the flat array at index==group; it silently misreads sparse/high groups (a bogus low addr can fake-pass a 2-bit calibration). The real path: `group=id/1000, sub=id%1000`; `map.find(group)` in the RB-tree @ `mgr+0x38` (node: left@0/parent@8/right@0x10/isnil@0x19/key@0x20); if found, read the node's **tag @ +0x28**: `tag==1` → block = `flat_base(mgr+0x28) + index(node+0x30)*125`; `tag==2` → block = heap ptr `(node+0x30)`; then `bit = block[sub>>3] >> (7-(sub&7))` (MSB-first). Group-not-found = flag OFF. The 10-digit area/entity flags (`10AABBCDDD`) are **tag-2 heap blocks** (that's why the flat-array shortcut read them as garbage). `read_event_flags.py` now does the full map+tag walk → validated: low registers unchanged, and `32009203`(Gael)=1, `1043379356`=0 (was a garbage `1` under the shortcut). CONSEQUENCE: RPM can now read every flag the quest state machines touch (registers + all gate/area/entity flags), so most mapping validation is doable OFFLINE against the live state — no per-step in-game test needed except to observe a register VALUE not present in the current save.
