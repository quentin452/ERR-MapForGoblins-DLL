...
**Key finding: the DLL-compiled grace table was ALREADY gone** (migrated to `capture_live_graces()` /
live `BonfireWarpParam` in a prior session — no `src/generated/*grace*` exists; `src/` only carries
"replaces the old baked grace_position_index" comments). What still carried the name was purely
**offline pipeline**: `data/grace_position_index.json` (generated by `tools/build_grace_index.py`,
pipeline stage `grace_index`), consumed ONLY by `massedit_common.resolve_location_id_at()`'s
stacked-tile (Nokron/Siofra, m12_02/m12_07) subtitle refinement — which writes `.MASSEDIT` `textId2`,
a **DEAD output** (the marker bake is an empty stub; every marker + subtitle is runtime). The kept
...
`STACKED_REGION_TILES` and degraded `resolve_location_id_at` to the tile-level `resolve_location_id`
(**signature kept** — ~12 orphaned Phase-5 generator scripts still import it; the x/y/z args are now
unused). Both edited scripts `py_compile` clean; no `grace_position_index`/`build_grace_index` refs
