...
needs an automated **`goods_count(id)`** read, which is currently blocked:

**⛔ `goods_count` via memory-walk is BLOCKED (proven 2026-07-03, ~14 boots + a before/after diff probe,
both reverted).** ELDEN RING stores held items as `{GaItemHandle, qty}` — the id only resolves via a
`GaItem` fetched through a MapItemMan hashmap, so no plain i32 scan finds a by-id count. Needs the
game's own count fn (or the handle-resolver + struct offsets) via Ghidra — same play that cracked
SERIALIZE_FN. **Prompt written and ready to run on Windows: `docs/re/windows_goods_count_re_prompt.md`**
(find `EquipInventoryData::GetItemQuantity` / shop "owned" check → RVA+AOB+convention). After it lands:
add `goods_count` (one guarded call) + `tools/rpc_tests/test_custom_item.py`.

**Meanwhile:** the bracket can be sanity-checked MANUALLY in-game (grant → save → empty `.mfg` → reload
→ eyeball inventory). Variant B (reserved-id item tolerated in the `.err`, no serialize hook) remains
the zero-RE fallback if this stalls.
