...
## ⇒ NEXT SESSION — active-mod MSB source: SWEEP REPLACED BY PATH CAPTURE (shipped, not deployed; 1st = verify on Golden Age)
...
package). Fix = read the ACTIVE mod's data.

**★ THE BLOCKER IS GONE — path capture replaces the memory sweep (user-approved direction,
implemented 2026-08-14, build-err green, NOT yet deployed/verified):**
- The CreateFileW observer (`loot_open_probe.cpp`, always armed) now records the EXACT RESOLVED
  path of every map file the game opens — `.msb.dcx`/`.msb`/`.mapbnd.dcx`/`.mapbnd`, successful
  opens only, deduped (`captured_map_files()`). ME3/UXM redirect BELOW CreateFileW → the
  captured path IS the active mod's real file (loader-agnostic ground truth), tile name free
  from the filename (LOD0 rule via the shared `parse_tile`).
- `load_resident_msbs` (resident_msb.cpp) is now PATH-DRIVEN: read + DCX-decompress each
  captured `.msb.dcx`/`.msb` from THAT exact path (`read_exact_file_decompressed` — slurp +
  dcx_decompress, no ancestor-walk re-resolution, no packed fallback; DFLT/zlib + KRAK + plain
  all handled), parse with `parse_msb(resident=false)`. Shared `emit_shapes` conversion.
  `.mapbnd` captures are recorded for the Oodle-join slice, not parsed yet.
- The old magic-scan + name-lookback + vtable walk stay in the file as **`resident_msb` RPC
  diagnostics ONLY** (`resident_msb` bare = retired scan count; `resident_msb dbg` = raw dump).
...
deployed to the GA install + LIVE-VERIFIED (session 18:54, user confirmed the chest marker
in-game):**
...
- **VERIFIED:** log `reading MSBs from GA\map\MapStudio + base E:\SteamLibrary\... (mod wins
  on its tiles)`; post-redirect 967 _00 tiles / 3334 treasures (was 154/1793); `vmap ename
  60 45 39` = 103 (was 0), `60 44 38` = 89 (was 0).
...
1. As maps stream: `[MAPOPEN]` (with `diag_map_opens = true` in the deployed ini) / RPC
   `resident_msb paths` must list the mod's real `GA\map\…` paths — NOT `E:\SteamLibrary\…`.
2. `[RESIDENTMSB] path source: N captured map files -> M parseable .msb, P parsed … -> K
   treasures (T tiles covered)` — then `[RESIDENTMSB] merged T resident tiles … over the disk
   parse`.
3. Markers on GA's tiles replace the vanilla ones (the whole point: Limgrave+ markers were the
   vanilla bake positions, wrong for GA).
4. ⚠ If the game opens mapbnds but NO `.msb.dcx` opens (some loaders read the MSB from inside
   the mapbnd): the Oodle-hook slice (below) is the fix, not more capture work.
5. ⚠ FRESHNESS: run `mfg.py rpc mfg_build` first — a stale DLL answers `ping` too; a rebuild
   needs a game restart to load.

**Not yet done (this thread, next session if (4) bites or as the natural follow-up):**
- **Oodle-hook slice:** the game decompresses the mapbnd through Oodle (DCXFileInterpreter/
  OodleDecompressionStream) → capture `dst` buffers starting with "MSB " via the existing
  `oodle_decompress_detour` (goblin_icon_harvest.cpp) and join them to the CreateFileW path by
  open-order/size; also covers MSBs that never exist as loose `.msb.dcx` (packed installs,
  mapbnd-embedded MSBs).
- Packed-install case: no loose files → no per-MSB CreateFileW → path source stays empty; the
...
retired for the freeze/miss reasons above, NOT for layout doubt. `resident_msb dbg` keeps
pinning the layout if the parse chain ever rejects the path-read files.
...
player's HAVOK BODY Vec3 directly, NOT the `+0x6C0` mirror (an output mirror physics reclaims → snap-back)
and NOT the engine `SetPos` (RE'd, but doesn't complete from the RPC thread). Findings:
`docs/re/linux_player_pos_write_setpos_re_findings.md`.

- **The working write (RE'd from `er_console_mod.dll` + live-verified):** `posObj = *(*(LocalPlayer+0x190)+
  0x68)`; body Vec3 @ `posObj+0x70/0x74/0x78` (X/Y/Z). Writing it MOVES the body and HOLDS. Live proof:
  `mem_write` body X += 20 → player moved +20 m, tile-local X -1.01→18.99, world 10750.99→10770.99, no
  snap-back. The body frame is havok/block-local, offset from the `+0x6C0` tile-local frame by a per-block
  origin, but a DELTA maps 1:1 → convert `body_target = target - (tile_now - body_now)`.
- **`goblin::warp::teleport_coords(x,y,z)`** (`src/goblin_warp.cpp`) now does that body write behind
...
- **Oodle route CHOSEN = RPC hybrid; bridge BUILT + DEPLOYED (`a98b4c8`).** New host helper
  `worldmap::dcx_decompress_bytes` + debug-RPC verb **`dcx_file <in.dcx> <out>`** decompress a raw DCX blob
  via the game's in-process Oodle under Proton (both builds green, deployed to `ERR/dll/offline/`). Tool
  gained `dump-inner` to stage a raw inner hkx. (The other routes — ooz `.so` / Wine C++ — are noted in
  findings §8 for the eventual whole-map BAKE, decided after material decode is proven.)
- **★ NEXT (turnkey, needs ER booted ONCE) — the material SPIKE:** (1) `cd tools/collision_offline && dotnet
  run -- dump-inner m14_00_00_00 _001000 /tmp/m14.hkx.dcx` (m14 = Academy, in Liurnia lake; raw KRAK). (2)
  `python tools/mfg.py repl --boot` → `dcx_file /tmp/m14.hkx.dcx /tmp/m14.hkx` (Oodle-decompress in-process;
  verify `mfg_build`+`status` in-world first). (3) offline `SoulsFormats.HKX.Read(/tmp/m14.hkx)` → geometry
  (decode confirmed) + `FSNPCustomParamCompressedMeshShape`; **reverse the material rider** — correlate
  `.primitives`/`sections` with the CustomParam `Unk68/Unk80/UnkA8` buffers, histogram material ids on the
  in-lake mesh vs a dry mesh (m10 Stormveil) to ID the Water/Swamp ordinal (cross-check via a runtime raycast
  material dump). Full recipe: findings §8 "Next run".
...
- **M1 sea-tag — global constant is WRONG; routed to proper per-region RE (2026-07-06f2).** User flagged
  ...
  measurement.
  - **UPDATE 2026-07-06 (Windows/Ghidra): BOTH Option 3 AND Option 2 ruled out → real path = a MATERIAL-tag.**
    `windows_water_level_source_re_findings.md`. Option 3 (`GXWaterHeightMap@GXSR`) = **GPU render-resource**
    (~7 ref-counted textures for the water interaction/wave sim, sub-object of `GXSceneContext` @ +0xBE20),
    not a CPU base-plane. Option 2 (a water-surface cast filter) = **also dead**: ER's collision filter
    (`CSCollisionFilter@CS` vt er+0x2b91d00, 128-layer matrix @ this+0x20, populate FUN_140c5dab0) shows the
    cast filter byte is just a 7-bit collision-LAYER index (0x5e=layer 94 vs terrain layer 2), and **water is
    NOT a collision layer** — it's a ground MATERIAL (`{...}_MatRatio_{Default,Grass,Water,Swamp}`); no water
    collision surface exists (WaterSurface/WaterMesh/… 0 hits). ⇒ **sea-tag = MATERIAL-based**: cast 0x5e
    (done) → hit triangle material via `hknpMaterialLibrary` (er+0x2ee36b0) → `sea = material ∈ {Water,Swamp}`.
    Follow-up RE: raycast-hit material extraction + the Water/Swamp material ids. Fallback: MSB water-plane Y.
