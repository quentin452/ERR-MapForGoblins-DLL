...
| **Name aliases** `goblin_name_aliases_en` | **M** (reclassified 2026-07-01, was A) | **engus FMG off disk** (GoodsName/WeaponName/ProtectorName/AccessoryName/GemName/NpcName/WorldMapPlaceName) | **DONE 2026-07-01 — migrated + baked table DELETED.** New `src/worldmap/name_fmg_en.cpp` reads the active install's `msg/engus/*.msgbnd.dcx` off disk (two-pass: loose mod bundles win, packed vanilla fills gaps — a first-hit-across-vanilla+mod mix was mislabeling ids), FMG-v2 group-table lookup keyed on the marker `name_id`. Deleted `goblin_name_aliases_en.{cpp,hpp}` + `generate_name_aliases_en_cpp` + CMake + `lookup_name_alias_en_utf8`. Offline 2754/2756 vs the old bake; IN-GAME confirmed on ERR (cross-language Godrick fix) + vanilla (`[NAMEEN] 9877` vanilla names, SIG 29/29). |
