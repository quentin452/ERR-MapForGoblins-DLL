...
| **Category exceptions** `goblin_category_exceptions` | A | (none — curated dict, no param/MSB source) | Authored core. (Already removed as a live-consumer; `item_icon_table.json` offline-only — see reordered PR 3.) |
| **Name aliases** `goblin_name_aliases_en` | **M** (reclassified 2026-07-01, was A) | **engus FMG off disk** (GoodsName/WeaponName/ProtectorName/AccessoryName/GemName/NpcName/WorldMapPlaceName) | **ASSESSED — migrate, do NOT dedup.** NOT authored: mechanically generated from FMG-sourced JSON (`items_database`/`npc_name_text_map`/`boss_list`), +900M enemy source already gone. The 4 per-profile copies are **byte-identical** (md5 `15abbe32…`) = ERR's names shipped into vanilla/Conv/ERTE → **wrong on any non-ERR mod**. Live `GetMessage` is language-locked (active language only), so English must come from reading `msg/engus/*.msgbnd.dcx` off disk via `read_game_file_decompressed` (infra exists) — needs a new C++ BND4+FMG parser (~250 lines) feeding the single consumer `goblin_overlay.cpp` search index; fall back to baked table on read failure. Acceptance: English search works on a **non-ERR** install. |
