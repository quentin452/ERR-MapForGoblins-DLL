...
| [param_override_loader_plan.md](param_override_loader_plan.md) — regulation.bin-free runtime param modding: generic `param_set_field` → name-addressed → boot-time override file | SCOPED 2026-07-03 (first step of the runtime-modding-framework direction); Slice 1 helper PROTOTYPED (`src/goblin_param_edit.{hpp,cpp}`, builds clean), not wired/exported/in-game-tested |
