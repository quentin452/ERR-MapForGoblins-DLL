...
**Status:** PR 0 DONE + MERGED to `master` (2026-07-01, in-game confirmed via log check: [SIG]
29/29 PASS, diag_loot_flags producing correct live output, no crash/error). **PR 1 DONE (branch
`feat/inject-icon-harvest`)** — icon-texture harvest/GPU registry extracted to
`src/goblin_icon_harvest.cpp`, builds clean (clang-cl+xwin), deployed + md5-verified, **NOT yet
in-game log-checked** (game wasn't running at deploy time). PRs 2-4 not started. Modeled
