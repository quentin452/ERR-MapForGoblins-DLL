...
**Status:** PR 0 + PR 1 + PR 2 DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 3 DONE (branch
`feat/inject-section-visibility`)** — per-section visibility + marker-clustering (8 non-contiguous
spans) extracted to `src/goblin_section_visibility.cpp`, builds clean, deployed + md5-verified,
**NOT yet in-game log-checked** (game wasn't running at deploy time). PR 4 not started. Modeled
...
| **3. Visibility + marker-clustering (as ONE unit)** ✅ DONE | `src/goblin_section_visibility.cpp` (new, ~675 lines): visibility globals + `is_section_hidden_ptr` (`:187-428`), cluster-config helpers + `seed_runtime_gates` (`:1140-1258`), the visibility/clustering half of "Overlay control API" (3 sub-spans `:1736-1955` minus save/reset/toast), per-category census (`:2118-2172`). Grace anchors turned out NOT coupled (own state, zero overlap) — correctly deferred to PR 4, the plan's original table wrongly lumped them in by proximity. 2 new tiny accessors (`icons_user_disabled`/`take_section_apply_req`) + `persist_settings` un-`static`'d, in `goblin_inject_shared.hpp` — same pattern as PR 0/1. | — | ~675 lines moved (8 non-adjacent spans) | DONE 2026-07-01, `feat/inject-section-visibility`, builds clean (clang-cl+xwin), deployed + md5-verified — **not yet in-game log-checked** |
