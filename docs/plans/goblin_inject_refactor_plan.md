...
**Status:** PR 0-3 + PR 4a DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 4b DONE (branch
`feat/inject-tutorial-popup`)** — TutorialParam row injection + `world_map_open()` extracted to
`src/goblin_tutorial_popup.cpp`, builds clean, deployed + md5-verified, **NOT yet in-game
log-checked** (game wasn't running at deploy time). PR 4c not started. Modeled
