...
**Status:** PR 0 + PR 1 + PR 2 DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 3 DONE + IN-GAME CONFIRMED + MERGED** (`feat/inject-section-visibility`)
— per-section visibility + marker-clustering (8 non-contiguous spans) extracted to
`src/goblin_section_visibility.cpp`. PR 4 not started. Modeled
