...
**Status:** PR 0-3 + PR 4a + PR 4b DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 4c DONE
(branch `feat/inject-grace-suppression`)** — native/DRAW-ONLY grace suppression extracted to
`src/goblin_grace_suppression.cpp`, builds clean, deployed + md5-verified, **NOT yet in-game
log-checked** (game wasn't running at deploy time). This is the LAST planned extraction PR — only
4d (intentional stay-behind) remains after this merges. Modeled
...
| **4a. World-position/grace-data domain** ✅ DONE | `src/goblin_world_position.cpp` (new, ~793 lines): dungeon→overworld legacy fold (`:60-176`), grace anchors + region/cluster naming + player world/map position + live grace capture + `marker_fragment_flag`/`marker_world_pos` wrappers (`:187-826`). 2 dead-but-still-referenced globals (`g_injected_row_ptrs`/`g_lot_backed_set`) sit between the two spans and stay behind (`apply_flag_or_pairs`/`injected_row_ptrs()` still reference them, even though that whole subsystem is dead — see finding below). | PR 0-3 landed + PR 4 scoping audit (2026-07-01) | ~793 lines moved (2 spans) | DONE + IN-GAME CONFIRMED + MERGED 2026-07-01, `feat/inject-world-position` (grace sprites/icons resolving correctly, no crash in our module — see note re: a pre-existing unrelated eldenring.exe crash, not caused by this PR) |
| **4b. TutorialParam row injection + `world_map_open`** ✅ DONE | `src/goblin_tutorial_popup.cpp` (new, ~354 lines, 3 spans `:89-134`+`:137-373`+`:433-463`): WorldMapPointParam readiness probe, TutorialParam row injection, `world_map_open()` (misplaced public utility called from `goblin_worldmap_probe.cpp`/`map_renderer.cpp`/`goblin_overlay.cpp`×2/`input_wndproc.cpp` — bundled here only by banner proximity). **Scope narrower than the plan guessed**: the toast-method A/B experiment (`g_toast_method`/`TOAST_METHOD_NAMES`/`TOAST_METHOD_COUNT`) and toast-delivery internals (`show_tutorial_popup_trampoline`/`seh_dispatch_toast`) STAY in `goblin_inject.cpp` — called ONLY by `show_toggle_banner`/`seh_fire_trampoline` (PR 4d's toast subsystem), moving them would just relocate half that hub for zero coupling reduction. | 4a | ~354 lines moved (3 spans) | DONE + IN-GAME CONFIRMED + MERGED 2026-07-01, `feat/inject-tutorial-popup` (TutorialParam expand + world_map_open resolve + map open/close all correct live) |
| **4c. Native/DRAW-ONLY grace suppression** ✅ DONE | `src/goblin_grace_suppression.cpp` (new, ~166 lines, `:152-297`): WarpPinData builder log hook + the DRAW-ONLY SetTo suppression hook. Already known-independent since before PR 3; fresh file-wide grep confirmed zero coupling — cleanest PR of the whole plan, first-try clean build. | 4a/4b | ~146 lines moved | DONE 2026-07-01, `feat/inject-grace-suppression`, builds clean (clang-cl+xwin), deployed + md5-verified — **not yet in-game log-checked** |
