...
**Status:** PR 0-3 + PR 4a + PR 4b DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 4c DONE + IN-GAME CONFIRMED + MERGED**
(`feat/inject-grace-suppression`) — native/DRAW-ONLY grace suppression extracted to
`src/goblin_grace_suppression.cpp`. **This was the LAST planned extraction PR — the plan is now
COMPLETE except for 4d, the intentional permanent stay-behind (not a PR).** Modeled
