...
29/29 PASS, diag_loot_flags producing correct live output, no crash/error). **PR 1 DONE + IN-GAME CONFIRMED + MERGED** (`feat/inject-icon-harvest`) —
icon-texture harvest/GPU registry extracted to `src/goblin_icon_harvest.cpp`. PRs 2-4 not
started. Modeled
