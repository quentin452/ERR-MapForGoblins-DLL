...
**Status:** PR 0 + PR 1 DONE + IN-GAME CONFIRMED + MERGED to `master`. **PR 2 DONE (branch
`feat/inject-item-classify`)** — taxonomy-based item/loot classification (9 functions, non-
contiguous span) extracted to `src/goblin_item_classify.cpp`, builds clean, deployed +
md5-verified, **NOT yet in-game log-checked** (game wasn't running at deploy time). PRs 3-4 not
started. Modeled
