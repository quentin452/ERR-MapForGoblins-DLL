...
- **Native world-map RENDER cull (stop the WorldMapDialog Scaleform draw) — OPEN, Windows; goal A now DEAD.**
  ...
  production.
- **ER in-combat state (gate the vmap like the native map) — STATIC PIN DONE, live-verify pending.** The
  native-map redirect (`native_map_redirect_linux_re_plan.md`) means the vmap can be stuck OPEN in combat.
  Ghidra pinned the flag: per-entity **`IsBattleState = *(int*)(*(ChrIns+0xC950)+0x30C)==6`** (AI FSM state 6);
  the player has no AI module so "in combat" is the **enemy aggregate** (any nearby enemy in state 6) — MFG
  reuses its CSFeman enemy-bar iteration (`goblin_enemy_names.cpp`) for `combat_active()`. Findings +
  live-checklist: `combat_state_gate_re_findings.md`; brief: `combat_state_gate_re_prompt.md`.
