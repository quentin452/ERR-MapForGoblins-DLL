...
| **Nav / warp** | `LUA_WARP`, `CS_LUA_EVENT_MANAGER` | `windows_grace_warppin_teleport`, `windows_grace_warppin_setto_abi`, `linux_player_pos_write_setpos` (why raw +0x6C0 writes don't teleport: it's an output mirror; SetPos = `er+0xdc6380`, r8/r9 args to RE) | Grace fast-travel (dev nav); coordinate teleport |
