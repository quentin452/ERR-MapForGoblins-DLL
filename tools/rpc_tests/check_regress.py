...
def write_status(runs, path):
    """Regenerate the git-tracked STATUS.md: the LATEST run of each test as a table. Derived from the
    (gitignored, local) ledger, so committing STATUS.md gives a visible, versioned test-status snapshot."""
    tests = sorted(runs)
    rows, npass = [], 0
    latest_ts = ""
    for t in tests:
        ...
        rows.append(f"| `{t}` | {'✅ PASS' if ok else '❌ FAIL'} | {r.get('passed')}/{r.get('total')} | {ts} |")
    lines = [
        "# RPC test status",
        "",
        "_Auto-generated by `tools/rpc_tests/check_regress.py` from the local run ledger "
        "(`results.jsonl`, gitignored). Reflects the LAST run of each test on the dev box. Do not edit._",
        "",
        "| Test | Status | Checks | Last run |",
        "|------|:------:|:------:|----------|",
        *rows,
        "",
        f"**Overall: {npass}/{len(tests)} tests passing**" + (f" (latest {latest_ts})" if latest_ts else "") + ".",
        "",
    ]
    try:
        ...
        print(f"[check-regress] STATUS.md write skipped: {e!r}")
...
def main():
    ...
    # Always refresh the git-tracked STATUS.md (full picture, independent of --test filter).
    status_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "STATUS.md")
    write_status(runs, status_path)
