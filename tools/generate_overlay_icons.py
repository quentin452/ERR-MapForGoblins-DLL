...
def main():
    ...
    # Embed the atlas as a compressed PNG blob (decoded once at runtime via stb_image)
    # rather than a raw RGBA array — keeps the generated .cpp small.
    import io
    _buf = io.BytesIO()
    atlas.save(_buf, format="PNG", optimize=True)
    atlas_png = _buf.getvalue()
    ...
    (OUT_DIR / "goblin_overlay_icons.hpp").write_text(
        "#pragma once\n"
        "// Generated by tools/generate_overlay_icons.py. Category icon atlas for the\n"
        "// in-game config overlay, embedded as a compressed PNG blob (decoded once at\n"
        "// runtime via stb_image). ATLAS_W/H/CELL + ICON_CELLS describe the decoded atlas.\n\n"
        "namespace goblin::overlay_icons\n{\n"
        "    extern const int ATLAS_W;\n"
        "    extern const int ATLAS_H;\n"
        "    extern const int CELL;\n"
        "    extern const unsigned char ATLAS_PNG[]; // PNG-encoded atlas (decode to RGBA)\n"
    ...
    with open(OUT_DIR / "goblin_overlay_icons.cpp", "w", encoding="utf-8", newline="\n") as f:
        ...
        f.write("const unsigned char ATLAS_PNG[] = {\n")
        for i in range(0, len(atlas_png), 32):
            f.write(",".join(str(b) for b in atlas_png[i:i + 32]) + ",\n")
        f.write("};\n")
        f.write(f"const int ATLAS_PNG_LEN = {len(atlas_png)};\n\n")
    ...
    print(f"[overlay-icons] atlas {atlas_w}x{atlas_h} ({n} icons, {atlas_w*atlas_h*4}B RGBA "
          f"-> {len(atlas_png)}B PNG), {len(key_cells)} key->cell -> {OUT_DIR}")
